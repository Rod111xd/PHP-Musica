...
";
	...
	$rCategory = $pC->selectPostsByCategory($_GET['category']);
	if(count($rCategory)==0){
		header("location:index.php");
	}
	echo "<title>MN - {$_GET['category']}</title>";
	$aC = new AdminControl();
	$iC = new ImageControl();
	...
	$i=0;
	echo "
	<div class='linearLeft'><h2>{$_GET['category']}</h2></div>
	...
	foreach ($rCategory as $post) {
		...
		if($actual>=$creationDate){
			...
			";
		if(count($rCategory)==1 || $i%2==1 || (count($rCategory)-1)==$i){
